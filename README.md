...
- **Capturas**: fotogramas en marcas concretas (`thumb frames`), por intervalos
  (`thumb interval`) o por cambios de escena (`thumb scene`).
...
| `split`      | Divide el vídeo por marcas de tiempo `--at hh:mm:ss[,hh:mm:ss,...]`; genera `nombre_000.ext`, `nombre_001.ext`… |
...
| `thumb frames`    | Captura fotogramas en marcas concretas (`--at hh:mm:ss,...`).                             |
| `thumb interval`  | Captura fotogramas a intervalos regulares (`--every N` segundos, opcional `--start/--end`). |
| `thumb scene`     | Captura fotogramas en los cambios de escena (`--scene` sensibilidad 0.1–0.5).             |

> El comando `thumb` se invoca como `pymedia frames/interval/scene …`.
